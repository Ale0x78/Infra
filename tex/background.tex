...
Cloaking techniques are broadly classified into server-side and client-side techniques~\cite{oest_inside_2018}.
Server-side techniques are stealthier, but they rely on limited information about the client.
Most server-side techniques rely on precompiled blocklists or allowlists of IP addresses, user agents, or referers (the page from which the link is visited, as identified by an HTTP header). 
