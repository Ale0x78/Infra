...
\DraftToDo{There is a theme coming up with this\ldots{} We need to double-check this is true when we look at JUST first-party scripts. But there is a takeaway: once the ecosystem adopts an API, it stays there. }
...
\DraftNote{Overall, from the categories in Crawlphish\cite{crawlphish}, fingerprinting is king, even if you follow the APIs they have in their tables. Regarding user interaction, we see much more DOM-related than browser permission requests. This is where we talk about that spike for bot detection that we see for generating random values.}
...
\DraftNote{FMI (think F1 score for clustering) is .97 for this. Adding noise (kits with only a single page) clusters them together\ldots{} so}
