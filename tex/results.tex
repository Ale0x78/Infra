...
\paperFinding{We see a mixture of obfuscation techniques in our dataset} Over 22,000 pages hide javascript evaluation behind a single eval state, but 744 pages have chains of evals. While we find 12,781 pages requesting a javascript object used for AES encryption, we only see 1,224 pages that call decrypt during the visit. We see 12.5\% of pages invoke a base64 or URL decoding API, frequently associated with obfuscation \cite{jsufp}. We also identify 125 scripts that are present at different eval depth\todowrite{What is an eval depth}
