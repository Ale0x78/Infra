...
\subsection{Phishing pages compared to their targets}
\subsection{Usage of emerging web functionality}
\subsubsection{WASM}
\subsubsection{Experimental APIs}
\todowrite{find that phishing pages are most likely to use experimental apis (as labeled by MDN) through 3rd party scripts. This is intuitive, as the only novelty they can add to first party behavior is through clocking, or mimicking.}
\subsection{Scripts in a phishing page}
\subsubsection{Crawlphish related APIs}
\subsubsection{Fingerprinting APIs}
\subsubsection{Non-deterministic behavior}
\subsection{Evaluating kit detection}
\subsection{Kit clusters in the wild}
