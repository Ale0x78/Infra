...
\subsection{Browser API traces for kit identification}
\paperFinding{Browser API sets help to identify the existence of a standard kit among different pages} For the 2,461 pages from 188 kits, we compute a 2,461 by 2,461 matrix of distances using the Jaccard index. The FMI (harmonic mean of TP and TN) comes to \textbf{0.97} with the v-measure of \textbf{0.87} and a silhouette score of \textbf{0.90}. Using a higher $\beta$ value for the V-measure, we can see that the model is more complete than homogeneous, meaning it tends to merge clusters. It should be noted that HDBSCAN gave us similar results as DBSCAN with a relatively low epsilon value ($\epsilon=0.1$), though, for a data source with larger quantities of data, we would recommend limiting the epsilon manually, setting it as a max similar to the ratio. In eight different cases, HDBSCAN clustered together pages with zero similarity. 

HDBSCAN allows for single kit clusters; given some points are distant enough from everything else, the algorithm will identify them as a solo cluster. Clustering all pages that we have kits for, even if they do not meet the requirement of having at least 2 pages, with support for single clusters enabled, we see an FMI of \textbf{0.92}, with a V-measure of \textbf{0.79}.
...
These clusters, on average, go on for 30 days ($\sigma=20$), with the longest one spanning 257 days, with 237 pages. 

\subsection{Browser API traces and Techniques}

\input{figures/brand_vs_phishing}
...
\paperFinding{Phishing pages employ experimental browser APIs that are not implemented by every version of the major browsers} MDN marks \MDNExperimentalTotal{} browser APIs, out of those, we see \numberofExperimentalAPIs{} in our dataset at least once. We find that the top 3 experimental APIs are \textit{NavigatorUAData.getHighEntropyValues()}, \textit{Keyboard.lock()}, and \textit{Scheduling.isInputPending()}  (\textbf{8,163/3,830/3,749} pages respecfully). Combined with the fact that not all versions of web browsers support these APIs, all of them make good candidates for cloaking conditions. \textit{Keyboard.lock} being executed when the page load would require a Transient user activation \todocite{MDN}, this makes it an excellent candidate for bot detection.
