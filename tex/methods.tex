...
VisibleV8 comes with built in post-processors that are able to isolate browser API calls per origin that they are called in, and the url of the script executing (in case of a script embedded in the HTML, the origin is the script url).
...
We describe API behavior to either be an increase, decrease, dip (a decrease followed by an increase), or a spike (an increase followed by a decrease).

We first isolate out 3,978 browser APIs that show up at least in 100 web pages, we use change point detection to identify the different sections within a time series of each API. For change point detection use use ruptures\todocite{ruptures}. We use an L1 cost function along with a Linearly penalized segmentation algorithm to find segmentation point of a deviation in the median.\todowrite{L2 (Change to the mean) makes more sense, no?}

To filter out noise results, we only consider APIs that have at least 2 but no more then 5 segments.
